...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    // *** Create a video without URL text to avoid special character issues ***
    await new Promise((resolve, reject) => {
      ...
      // Create a simple colored video WITHOUT any problematic text
      const cmd = `ffmpeg -y -f lavfi -i color=c=${color}:s=640x360:d=10 -vf "drawtext=text='Video Generated':fontcolor=white:fontsize=24:x=(w-text_w)/2:y=(h-text_h)/2" -c:v libx264 -pix_fmt yuv420p "${videoPath}"`;
      ...
      exec(cmd, (error, stdout, stderr) => {
        if (error) {
          ...
          // If the previous command failed, try an even simpler one
          const fallbackCmd = `ffmpeg -y -f lavfi -i color=c=${color}:s=640x360:d=10 -c:v libx264 -pix_fmt yuv420p "${videoPath}"`;
          ...
          exec(fallbackCmd, (fallbackError) => {
            ...
            console.log('Video created successfully with fallback command');
            resolve();
          });
          return;
        }
    ...
    // Send back the video information with ALL required properties for frontend compatibility
    const clipData = {
      timestamp: "00:00:00 - 00:00:10",
      headline: "Video for: " + videoUrl.substring(0, 30) + "...",
      file: `/uploads/${videoFile}`,
      videoUrl: `/uploads/${videoFile}`,
      captions: {
        tiktok: "🎬 Generated video from YouTube URL #video",
        twitter: "Check out this video clip I generated",
        linkedin: "Generated content from a YouTube video source"
      }
    };
    ...
    // Create an ultra-simple fallback video as last resort
    try {
      const timestamp = Date.now();
      ...
      await new Promise((resolve, reject) => {
        const cmd = `ffmpeg -y -f lavfi -i color=c=blue:s=640x360:d=10 -c:v libx264 -pix_fmt yuv420p "${fallbackPath}"`;
        
        exec(cmd, (cmdError) => {
          if (cmdError) {
            console.error('Fallback video creation failed:', cmdError);
            reject(cmdError);
            return;
          }
      ...
      // Return the fallback video with ALL required properties for frontend compatibility
      const fallbackClip = {
        timestamp: "00:00:00 - 00:00:10",
        headline: "Fallback Video",
        file: `/uploads/${fallbackFile}`,
        videoUrl: `/uploads/${fallbackFile}`,
        captions: {
          tiktok: "🚫 Error processing video - fallback created #error",
          twitter: "This is a fallback video due to processing error",
          linkedin: "Fallback content due to error in video processing workflow"
        }
      };
